...
declare global {
    interface Window {
        OmiseCard: {
            configure: (config: { publicKey: string; currency?: string; frameLabel?: string; submitLabel?: string; buttonLabel?: string }) => void
            open: (config: { amount: number; onCreateTokenSuccess: (token: string) => void; onFormClosed: () => void }) => void
        }
    }
}
...
function PricingContent() {
    ...
    // Configure Omise when script loads
    const configureOmise = useCallback(() => {
        if (window.OmiseCard) {
            window.OmiseCard.configure({
                publicKey: process.env.NEXT_PUBLIC_OMISE_PUBLIC_KEY || '',
                currency: 'THB',
                frameLabel: 'Popcorn Creator',
    ...
    useEffect(() => {
        const verifyPayment = async () => {
            ...
            if (searchParams.get('omise_callback') === 'true' && chargeId) {
                setShowSuccess(true)

                try {
                    const res = await fetch('/api/omise/verify', {
                        method: 'POST',
            ...
            onCreateTokenSuccess: async (token: string) => {
                try {
                    ...
                    if (data.authorizeUri) {
                        // 3DS required - redirect to authorize
                        ...
                        alert(data.failureMessage || 'การชำระเงินล้มเหลว กรุณาลองใหม่')
                    }
                } catch (error) {
